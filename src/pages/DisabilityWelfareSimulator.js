...
import { computeSeries } from "../calc/computePoint";
    ...
    const t = setTimeout(() => {
      try {
        const out = computeSeries({
          household: { head, spouseEnabled, spouse, children },
          scenario: taxModel,
          tables: {
            emp: empStatic,
            basicIT: basicITStatic,
            basicLT: basicLTStatic,
            socialU40: socialU40Static,
            socialO40: socialO40Static,
            taxTable: taxTableStatic,
            configs: {
          ...
          sweep: { min: SWEEP_MIN_X, max: SWEEP_MAX_X, step: SWEEP_STEP },
        });
        if (cancelled) return;
